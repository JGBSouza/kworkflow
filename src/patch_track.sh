...
function show_patches_dashboard()
{
  ...
  print_patches_dashboard 'patches_array'
}
...
function print_patches_dashboard()
{
  ...
  printf "%-${id_width}s|%-${date_width}s|%-${time_width}s|%-${status_width}s|%-${title_width}s\n" "ID" "Date" "Time" "Status" "Title"
  printf "%-${columns}s\n" | tr ' ' '-'

  # Print rows
  for mail in "${!_patches_array[@]}"; do
    IFS='|' read -r id date time status title <<< "${_patches_array[$mail]}"
    printf "%-${id_width}s|%-${date_width}s|%-${time_width}s|%-${status_width}s|%-${title_width}s\n" "$id" "$date" "$time" "$status" "$title"
  done
...
{
  local long_options='id:,dashboard,set-status::,from:,before:,after:,'
