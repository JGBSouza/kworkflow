...
# kw_mail tables
declare -g DATABASE_TABLE_GROUP='mail_group'
declare -g DATABASE_TABLE_CONTACT='mail_contact'
declare -g DATABASE_TABLE_CONTACT_GROUP='mail_contact_group'
...
# This function check over the kw mail groups options and
# calls the proper handling function for every option.
#
# Returns:
# returns 0 if successful, non-zero otherwise
function kw_mail_groups()
{
  if [[ -n "${options_values['GROUPS_CREATE']}" ]]; then
    create_new_kw_mail_group "${options_values['GROUP']}"
    return "$?"
  fi

  if [[ -n "${options_values['GROUPS_APPEND']}" ]]; then
    append_kw_mail_contacts "${options_values['GROUPS_APPEND']}" "${options_values['GROUP']}"
    return "$?"
  fi

  if [[ -n "${options_values['GROUPS_RENAME']}" ]]; then
    rename_kw_mail_group "${options_values['GROUP']}" "${options_values['GROUPS_RENAME']}"
    return "$?"
  fi

  if [[ -n "${options_values['GROUPS_REMOVE_EMAIL']}" ]]; then
    remove_kw_mail_contact "${options_values['GROUPS_REMOVE_EMAIL']}"
    return "$?"
  fi

  if [[ -n "${options_values['GROUPS_REMOVE']}" ]]; then
    remove_kw_mail_group "${options_values['GROUPS_REMOVE']}"
    return "$?"
  fi
...
function create_new_kw_mail_group()
{
  ...
  result="$(select_from_where "$DATABASE_TABLE_GROUP" 'name' '' 'condition_array')"
  ...
  insert_into $DATABASE_TABLE_GROUP '(name)' "$values" || exit_msg 'Error while creating new group'
...
function append_kw_mail_contacts()
{
  ...
  condition_array=(['name']="${group_name}")
  group_id="$(select_from_where "$DATABASE_TABLE_GROUP" 'id' '' 'condition_array')"
...
function check_string_chars_precedence()
{
  local first_char="$1"
  local second_char="$2"

  if [[ "$less_than_char_pos" -eq 0 || "$less_than_char_count" -eq 0 ]]; then
    return 22 #EINVAL
  elif [[ "$less_than_char_pos" -gt "$greater_than_char_pos" ]]; then
    return 22
  elif [[ "$greater_than_char_count" -ne 1 ]]; then
    return 22
  elif [[ "$less_than_char_count" -ne 1 ]]; then
    return 22
  fi
}

# This function check if the contacts in contacts_array are valid by 
# checking if their emails are not already in the database
#
# @contacts_array: An associative array formed as "<CONTACT_EMAIL>:<CONTACT_NAME>"
#
# Returns:
...
function validate_contacts()
{
  ...
  for email in "${!contacts_array[@]}"; do
    ...
    result="$(select_from_where "$DATABASE_TABLE_CONTACT" 'name' '' 'condition_array')"
...
function add_contacts()
{
  ...
  for email in "${!contacts_array[@]}"; do
    ...
    insert_into $DATABASE_TABLE_CONTACT '(name, email)' "$values" || return 22 #EINVAL
  done
...
function add_contact_group()
{
  ...
  for email in "${!contacts_array[@]}"; do
    condition_array=(['email']="${email}")
    contact_id="$(select_from_where "$DATABASE_TABLE_CONTACT" 'id' '' 'condition_array')"
    values="$(format_values_db 2 "$contact_id" "$group_id")"
    insert_into "$DATABASE_TABLE_CONTACT_GROUP" '(contact_id, group_id)' "$values" || return 22 #EINVAL
  done
...
function remove_contact()
{
  ...
  return $(remove_from "$DATABASE_TABLE_CONTACT" 'condition_array')
}
...
function remove_contact_groups()
{
  ...
  return $(remove_from "$DATABASE_TABLE_CONTACT_GROUP" 'condition_array')
}
...
function rename_kw_mail_group()
{
  ...
  replace_into $DATABASE_TABLE_GROUP '("id","name")' "('${group_id}','${new_name}')" || return 22 #EINVAL
...
function remove_group()
{
  ...
  condition_array=(['name']="${group_name}")
  return $(remove_from "$DATABASE_TABLE_GROUP" 'condition_array')
}
...
function remove_contacts_without_group()
{
  ...
  _contacts_id_list="$(select_from "$DATABASE_TABLE_CONTACT" 'id')"

  for id in $_contacts_id_list; do

    condition_array=(['contact_id']="${id}")
    contact_groups="$(select_from_where "$DATABASE_TABLE_CONTACT_GROUP" 'group_id' '' 'condition_array')"
    ...
    remove_from "$DATABASE_TABLE_CONTACT" 'condition_array' || return 22 #EINVAL
  done
...
function parse_mail_options()
{
  ...
  options_values['GROUPS_CREATE']=''
  options_values['GROUPS_RENAME']=''
  options_values['GROUPS_APPEND']=''
  options_values['GROUPS_REMOVE']=''
  options_values['GROUPS_REMOVE_EMAIL']=''
  ...
  while [[ "$#" -gt 0 ]]; do
    case "$1" in
      --groups)
        ...
        ;;
      --create)
        options_values['GROUPS_CREATE']=1
        options_values['GROUP']="$2"
        shift 2
        ;;
      --append)
        options_values['GROUPS_APPEND']="$2"
        shift 2
        ;;
      --remove)
        options_values['GROUPS_REMOVE']="$2"
        shift 2
        ;;
      --remove-email)
        options_values['GROUPS_REMOVE_EMAIL']="$2"
        shift 2
        ;;
      --rename)
